...
<div class="recommendations_content">
	<div id="tab_content_1" class="single_content">
		...
		<form name="bb_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
			...
			<p>
				<b><?php echo $bb_lang['max_comments']; ?></b> 
				<select name="bb_max_comments"> 
				<?php
				$bb_max_comments = get_option('bb-max-comments');
				for ($i=1; $i<=10; $i++) {
					if($bb_max_comments == $i) { $selected = ' selected="selected"'; } else { $selected = ''; }
					echo '<option value="'.$i.'"'.$selected.'>'.$i.'</option>';
				}
				?>
				</select><br />
	...
	<form method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
		...
		<tr>
			<td><input type="checkbox" name="bb_sharing_services[]" value="1" <?php if (in_array("1", $services_array)) { echo 'checked="checked" '; } ?>/> <img src="<?php echo $plug_url."/images/icons/delicious.png" ?>" alt="delicious" /> Delicious</td>
		...
		<tr>
			...
			<td><input type="checkbox" name="bb_sharing_services[]" value="9" <?php if (in_array("9", $services_array)) { echo 'checked="checked"'; } ?>/> <img src="<?php echo $plug_url."/images/icons/stumbleupon.png" ?>" alt="stumbleupon" /> StumbleUpon</td>
		</tr>
		<tr>
			<td colspan="4"></td>
		</tr>
	...
	<div id="tab_content_4" class="single_content">
		...
		<p></p>
		<ul>
			<li><b><?php echo $bb_lang['author']; ?>:</b> <a href="http://www.svil4ok.com/" target="_blank"><?php echo $bb_lang['name']; ?></a></li>
			<li><b><?php echo $bb_lang['url']; ?>:</b> <a href="http://www.svil4ok.com/wordpress-plugins/bottom-bar/" target="_blank">http://www.svil4ok.com/wordpress-plugins/bottom-bar/</a></li>
			<li><b><?php echo $bb_lang['version']; ?>:</b> 0.1.7</li>
			<li><b><a href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=svil4ok%40gmail%2ecom&item_name=WordPress+Bottom+Bar&item_number=Support+Open+Source+Project&no_shipping=0&no_note=1&tax=0&currency_code=USD&lc=US&bn=PP+DonationsBF&charset=UTF%2d8"><?php echo $bb_lang['donate']; ?></a></b></li>
		</ul>
	</div>
</div>
</div>